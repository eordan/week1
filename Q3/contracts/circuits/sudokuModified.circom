...
template sudoku() {
    ...
    component rpPuzzle[9][9];
    component rpSltion[9][9];

    for (var i=0; i<9; i++) {
        for (var j=0; j<9; j++) {
            rpPuzzle[i][j] = RangeProof(32);
            rpSltion[i][j] = RangeProof(32);
            rpPuzzle[i][j].range[0] <== 0;
            rpPuzzle[i][j].range[1] <== 9;
            rpSltion[i][j].range[0] <== 0;
            rpSltion[i][j].range[1] <== 9;
            rpPuzzle[i][j].in <== puzzle[i][j];
            rpSltion[i][j].in <== solution[i][j];
            assert(rpPuzzle[i][j].out == 1);
            assert(rpSltion[i][j].out == 1);
