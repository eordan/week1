...
describe("HelloWorld", function () {
    ...
    it("Should return true for correct proof", async function () {
        ...
        // Generates a zk Proof from input
        const { proof, publicSignals } = await groth16.fullProve({"a":"1","b":"2"}, "contracts/circuits/HelloWorld/HelloWorld_js/HelloWorld.wasm","contracts/circuits/HelloWorld/circuit_final.zkey");

        // Displays the result of multiplying the values of a and b in the console
        console.log('1x2 =',publicSignals[0]);

        // Calls function 'unstringifyBigInts' to unstringify 'publicSignals'
        const editedPublicSignals = unstringifyBigInts(publicSignals);

        // Calls function 'unstringifyBigInts' to unstringify 'proof'
        const editedProof = unstringifyBigInts(proof);

        // Generates the parameters of the call for the 'verifyProof' function
        const calldata = await groth16.exportSolidityCallData(editedProof, editedPublicSignals);

        // Parses a string 'calldata' and genertes an array of nine values     
        const argv = calldata.replace(/["[\]\s]/g, "").split(',').map(x => BigInt(x).toString());

        // Assignes the first two values to a constant 'a'     
        const a = [argv[0], argv[1]];

        // Assignes the third to sixth values to a constant 'b'
        const b = [[argv[2], argv[3]], [argv[4], argv[5]]];

        // Assignes the seventh two eightth values to a constant 'c'
        const c = [argv[6], argv[7]];

        // Assignes the ninth value to a constant 'Input'
        const Input = argv.slice(8);

        // Calls function 'verifyProof' to verify if the proof is true
...
describe("Multiplier3 with Groth16", function () {
    ...
    it("Should return true for correct proof", async function () {
        //[assignment] insert your script here
        const { proof, publicSignals } = await groth16.fullProve({"in1":"4","in2":"5","in3":"8"}, "contracts/circuits/Multiplier3/Multiplier3_js/Multiplier3.wasm","contracts/circuits/Multiplier3/circuit_final.zkey");
        console.log('4x5x8 =',publicSignals[0]);
        const editedPublicSignals = unstringifyBigInts(publicSignals);
        const editedProof = unstringifyBigInts(proof);
        const calldata = await groth16.exportSolidityCallData(editedProof, editedPublicSignals);
        const argv = calldata.replace(/["[\]\s]/g, "").split(',').map(x => BigInt(x).toString());  
...
describe("RangeProof with Groth16", function () {
    ...
    it("Should return true for correct proof", async function () {
        //[assignment] insert your script here
        const { proof, publicSignals } = await groth16.fullProve({"in": "5", "range": [5, 16]}, "contracts/circuits/RangeProofCurcuit/RangeProofCurcuit_js/RangeProofCurcuit.wasm","contracts/circuits/RangeProofCurcuit/circuit_final.zkey");
        console.log('5 <= 5 <= 16, Output is',publicSignals[0]);
        const editedPublicSignals = unstringifyBigInts(publicSignals);
        const editedProof = unstringifyBigInts(proof);
        const calldata = await groth16.exportSolidityCallData(editedProof, editedPublicSignals);
        const argv = calldata.replace(/["[\]\s]/g, "").split(',').map(x => BigInt(x).toString());  
...
describe("Multiplier3 with PLONK", function () {
    ...
    it("Should return true for correct proof", async function () {
        //[assignment] insert your script here
        const { proof, publicSignals } = await plonk.fullProve({"in1":"4","in2":"5","in3":"8"}, "contracts/circuits/_plonkMultiplier3/Multiplier3_js/Multiplier3.wasm","contracts/circuits/_plonkMultiplier3/circuit_final.zkey");
        console.log('4x5x8 =',publicSignals[0]);
        const editedPublicSignals = unstringifyBigInts(publicSignals);
        const editedProof = unstringifyBigInts(proof);
